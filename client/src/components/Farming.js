...
const Farming = () => {
    ...
    const fetchFarmingNews = async () => {
        try {
            const response = await axios.get('http://localhost:5000/api/posts?category=agriculture');
            if (Array.isArray(response.data)) {
                // Sort posts by date in descending order (newest first)
                setFarmingNews(response.data.sort((a, b) => new Date(b.date) - new Date(a.date)));
            } else if (response.data.posts) {
                setFarmingNews(response.data.posts.sort((a, b) => new Date(b.date) - new Date(a.date)));
            } else {
                throw new Error('Unexpected response format');
            }
        } catch (error) {
            console.error('Error fetching farming news:', error);
            setError('Failed to fetch farming news. Please try again later.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const currentNews = farmingNews.slice(indexOfFirstPost, indexOfLastPost);
    const totalPages = Math.ceil(farmingNews.length / postsPerPage);
            ...
            <div className="w-full md:w-2/3 p-2">
                <div className="xs:p-0 lg:p-4 sm:p-2 rounded-sm bg-white shadow-lg overflow-hidden">
                    <nav className="text-gray-500 mb-3 text-sm md:text-base truncate m-1">
                        ...
                        <Link to="/farming" className="hover:underline italic">Farming</Link>
                    </nav>
                    <h1 className="text-xl md:text-2xl font-bold mb-4 ml-1">Farming News</h1>
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-wrap">
                            {currentNews.length > 0 ? (
                                ...
                                <p>No posts available for the agriculture category.</p>
                            )}
                        </div>
                    )}
