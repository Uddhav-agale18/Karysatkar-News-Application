...
const Jalna = () => {
    const [jalnaNews, setJalnaNews] = useState([]);
    ...
    useEffect(() => {
        const fetchJalnaNews = async () => {
            try {
                const response = await axios.get('http://localhost:5000/api/posts?category=jalna-district');
                // Sort posts by date in descending order to show the most recent posts first
                const sortedPosts = response.data.posts.sort((a, b) => new Date(b.date) - new Date(a.date));
                setJalnaNews(sortedPosts || []);
            } catch (err) {
                console.error('Error fetching Jalna district news:', err);
                setError('Failed to load Jalna district news. Please try refreshing the page.');
            } finally {
                setLoading(false);
            }
        };
    ...
    const currentItems = jalnaNews.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(jalnaNews.length / itemsPerPage);
            ...
            <div className="w-full md:w-2/3 p-2">
                <div className="xs:p-0 lg:p-4 sm:p-2 rounded-sm bg-white shadow-lg overflow-hidden">
                    <nav className="text-gray-500 mb-3 text-sm md:text-base truncate m-1">
                        ...
                        <Link to="/jalna-district" className="hover:underline italic">Jalna District</Link>
                    </nav>
                    <h1 className="text-xl md:text-2xl font-bold mb-4 ml-1">Jalna District News</h1>
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-wrap">
                            {currentItems.length > 0 ? (
                                ...
                                <p>No posts available for Jalna District.</p>
                            )}
                        </div>
                    )}
