...
const Environment = () => {
    ...
    const fetchEnvironmentNews = useCallback(async () => {
        try {
            const response = await axios.get('http://localhost:5000/api/posts?category=weather');
            const posts = response.data.posts || [];

            // Reverse the posts to show the newest one first
            setEnvironmentNews(posts.reverse());
        } catch (error) {
            setError('Failed to fetch environment news. Please try again later.');
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const currentItems = environmentNews.slice(indexOfFirstPost, indexOfLastPost);
    const totalPages = Math.ceil(environmentNews.length / itemsPerPage);

    // Handle post click for detailed view
    ...
    const handleAddPost = async (newPost) => {
        try {
            ...
            // Prepend the new post to the top and reset the current page to 1
            setEnvironmentNews((prevPosts) => [addedPost, ...prevPosts]);
            setCurrentPage(1); // Go back to the first page after adding a new post
        } catch (error) {
            ...
            <div className="w-full md:w-2/3 p-2">
                <div className="xs:p-0 lg:p-4 sm:p-2 rounded-sm bg-white shadow-lg overflow-hidden">
                    <nav className="text-gray-500 mb-3 text-sm md:text-base truncate m-1">
                        ...
                        <Link to="/weather" className="hover:underline italic">Weather</Link>
                    </nav>
                    <h1 className="text-xl md:text-2xl font-bold mb-4 ml-1">Environment News</h1>
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-wrap">
                            {currentItems.length > 0 ? (
                                ...
                                <p>No news available for the environment category.</p>
                            )}
                        </div>
                    )}
